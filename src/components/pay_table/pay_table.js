...
function insertDefaultOddsToPayTable()
{
  ...
  const cherryDecimalOdds = roundToPrecision(permutateOdds(DEFAULT_ODDS_CHERRY, 1), 4);
  const cherriesDecimalOdds = roundToPrecision(permutateOdds(DEFAULT_ODDS_CHERRY, numOfReelsForPayTable), 4);
  const barsDecimalOdds = roundToPrecision(permutateOdds(DEFAULT_ODDS_BAR, numOfReelsForPayTable), 4);
  const chipsDecimalOdds = roundToPrecision(permutateOdds(DEFAULT_ODDS_CHIPS, numOfReelsForPayTable), 4);
  const gemsDecimalOdds = roundToPrecision(permutateOdds(DEFAULT_ODDS_GEM, numOfReelsForPayTable), 4);
  const sevensDecimalOdds = roundToPrecision(permutateOdds(DEFAULT_ODDS_7, numOfReelsForPayTable), 4);

  // Formatting odds into permutated rounded percentages (out of 100):
  document.getElementById("cherry_set_odds").innerText = roundToPrecision(cherryDecimalOdds * 100, 4);
  document.getElementById("cherries_set_odds").innerText = roundToPrecision(cherriesDecimalOdds * 100, 4);
  document.getElementById("bars_set_odds").innerText = roundToPrecision(barsDecimalOdds * 100, 4);
  document.getElementById("chips_set_odds").innerText = roundToPrecision(chipsDecimalOdds * 100, 4);
  document.getElementById("gems_set_odds").innerText = roundToPrecision(gemsDecimalOdds * 100, 4);
  document.getElementById("seven_set_odds").innerText = roundToPrecision(sevensDecimalOdds * 100, 4);
}
