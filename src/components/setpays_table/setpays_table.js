...
function mapValueToSlotsDictionaryField(valueID, valueToMap, thisPayValueDictionary)
{
  ...
  {
    console.log("DICTIONARY-ERR: Could not map pay value change due to valueID not being of type STRING.");
  }
  
}

// Attempts to change a pay value using the specified lookup and tag data:
function attemptPayValueChange(inputValue, inputLookupID, outputLookupID, payValueDictionary)
{
  console.log("I am executing!");

  if (inputValue != null && inputValue.length > 0 && outputLookupID != null && payValueDictionary.constructor == Object) 
  {
    ...
    if (isAPositiveInteger(inputValue, true)) 
    {
      ...
      mapValueToSlotsDictionaryField(outputLookupID, inputValue, payValueDictionary);

      $("#" + inputLookupID).prop("value", "");

      console.log("new SymbolPays dictionary: \nSevens: " + SymbolPays.Seven + "\nBar: " + SymbolPays.Bar + "\nCherry: " + SymbolPays.Cherry + "\nChips: " + SymbolPays.Chips + "\nGem: " + SymbolPays.Gem + "\nWild: " + SymbolPays.Wild);
    ...
    {
      console.log(
        "ERROR-SETPAYS: New set value for " + inputLookupID.toString() + " is invalid. Please ensure the provided value is >= 0."
      );
...
function changePaysInPayTable(
  ...
  const cherryInpValue = strCherryInpID != null ? $("#" + strCherryInpID).val() : null;
  const barInpValue = strBarInpID != null ? $("#" + strBarInpID).val() : null;
  const chipInpValue = strChipInpID != null ? $("#" + strChipInpID).val() : null;
  const gemInpValue = strGemInpID != null ? $("#" + strGemInpID).val() : null;
  const sevenInpValue = strSevenInpID != null ? $("#" + strSevenInpID).val() : null;
  ...
  // Cherries:
  let cherryStatusCode = attemptPayValueChange(cherryInpValue, strCherryInpID, strCherryOutID, symbolPaysDictionary);
  ...
  if (cherryStatusCode == 0)
  {
    ...
    if (strSingleCherryOutID != null) 
    {
      ...
      mapValueToSlotsDictionaryField(strSingleCherryOutID, calculatedWildValue, symbolPaysDictionary);
    }
  }

  // Bars:
  let barStatusCode = attemptPayValueChange(barInpValue, strBarInpID, strBarOutID, symbolPaysDictionary);
  updateOperationCounters(ioCounter, barStatusCode);

  // Chips:
  let chipStatusCode = attemptPayValueChange(chipInpValue, strChipInpID, strChipOutID, symbolPaysDictionary);
  updateOperationCounters(ioCounter, chipStatusCode);

  // Gems:
  let gemStatusCode = attemptPayValueChange(gemInpValue, strGemInpID, strGemOutID, symbolPaysDictionary);
  updateOperationCounters(ioCounter, gemStatusCode);

  // Sevens:
  let sevenStatusCode = attemptPayValueChange(sevenInpValue, strSevenInpID, strSevenOutID, symbolPaysDictionary);
  ...
  {
    setChangeStatus("setpays_chng_success", "Changes failed. Credits must be positive whole numbers.");
  }
}
